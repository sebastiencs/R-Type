...
Sprite::Sprite(const std::string & img, const Transformation & t, IGraphicEngine_SharedPtr &&engine, const Color& color) : color(color)
{
	_transformation = t;
	_id = img;
	_visible = true;
	...
	this->img = img;
	if (engine) {
		this->engine = dynamic_cast<GraphicEngine*>(engine.get());
		if (!engine)
